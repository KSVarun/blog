...
Yes they are similar as the file the same entity.
...
Should the hash be created using js? The answer is **no**.
...
So ideally the browser should not be responsible for creating MD5 hash but rather it's users responsibality to create the hash and share it with a remote server along with the file to verify it's integrity
